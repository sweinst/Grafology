...
// Apple Clang 12.0.0 does not have the <coroutine> header
// so we use the reference implementation of proposal P2168
// see https://raw.githubusercontent.com/lewissbaker/generator/e7c6c1c/include/__generator.hpp
...
namespace std {
  ...
  class __promise_base_alloc {
   ...
   public:
    template <typename... _Args>
    static void*
    operator new(std::size_t __frameSize, std::allocator_arg_t, _Alloc __alloc, _Args&...) {
    ...
    static void*
    operator new(std::size_t __frameSize, _This&, std::allocator_arg_t, _Alloc __alloc, _Args&...) {
      return __promise_base_alloc::operator new(
          __frameSize, std::allocator_arg, std::move(__alloc)
      );
    }
  ...
  struct __generator_promise<generator<_Ref, _Value, _Alloc>, _ByteAllocator, _ExplicitAllocator>
    ...
    yield_value(std::ranges::elements_of<_Rng>&& __x) {
      static_assert(
          !_ExplicitAllocator,
          "This coroutine has an explicit allocator specified with std::allocator_arg so an "
          "allocator needs to be passed "
