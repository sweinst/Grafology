...
namespace LondonTube {
  ...
  std::set<Line> lines{
      {0, "Metropolitan"},       {1, "Bakerloo"}, {2, "Central"},   {3, "Victoria"},
      {4, "Waterloo & City"},    {5, "Jubilee"},  {6, "Northern"},  {7, "Piccadilly"},
      {8, "Hammersmith & City"}, {9, "Circle"},   {10, "District"}, {11, "East London"},
  };
  ...
  double Station::distance_from(const Station& s) const {
    ...
    double h =
        std::sqrt((1 - std::cos(lat2 - lat1) + std::cos(lat1) * std::cos(lat2) * (1 - std::cos((lon2 - lon1)))) / 2.0);
