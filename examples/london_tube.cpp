...
std::set<Line> lines{
    {0, "Metropolitan"}, {1, "Bakerloo"},        {2, "Central"},
    {3, "Victoria"},     {4, "Waterloo & City"}, {5, "Jubilee"},
    {6, "Northern"},     {7, "Piccadilly"},      {8, "Hammersmith & City"},
    {9, "Circle"},       {10, "District"},       {11, "East London"},
};
std::set<Connection> connections{
    ...
    {37, 38, 1.27, 1, 115.19999999999999, 123.60000000000001,
     123.60000000000001},
    ...
    {97, 96, 1.34, 2, 121.79999999999998, 166.79999999999998,
     133.20000000000002},
    ...
    {262, 184, 1.21, 10, 109.80000000000001, 157.79999999999998,
     123.60000000000001},
...
std::optional<Station> Station::from_id(g::vertex_t id) {
  ...
  return (it == stations.end()) ? std::optional<Station>{}
                                : std::optional<Station>{*it};
}

std::optional<Station> Station::from_name(const std::string& name) {
  auto it = std::find_if(stations.begin(), stations.end(),
                         [&name](const Station& s) { return s._name == name; });
  return (it == stations.end()) ? std::optional<Station>{}
                                : std::optional<Station>{*it};
}
...
double Station::distance_from(const Station& s) const {
  ...
  double h = std::sqrt(
      (1 - std::cos(lat2 - lat1) +
       std::cos(lat1) * std::cos(lat2) * (1 - std::cos((lon2 - lon1)))) /
      2.0);
  return 2.0 * r * std::asin(std::min(1.0, h));
}

std::optional<Connection> Connection::from_id(g::vertex_t start,
                                              g::vertex_t end) {
  ...
  return (it == connections.end()) ? std::optional<Connection>{}
                                   : std::optional<Connection>{*it};
}
